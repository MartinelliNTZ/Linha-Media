...
class LinhaPerpendicularPoligonoAlgorithm(QgsProcessingAlgorithm):
    ...
    def name(self):
        return "curveconnection"

    def displayName(self):
        return self.tr("Conexão de Curvas")
    ...
    def _apply_crossing_filter_to_valid_connections(
        ...
        for feature_id, record in enumerate(valid_records):
            ...
            for candidate_id in crossing_index.intersects(geometry.boundingBox()):
                ...
                record_key = LinhaPerpendicularPoligonoAlgorithm._pair_connection_sort_key(
                    record,
                    record_positions,
                )
                candidate_key = (
                    LinhaPerpendicularPoligonoAlgorithm._pair_connection_sort_key(
                        candidate_record,
                        record_positions,
                    )
                )
    ...
    def _connection_crosses_standardized_layer(
        ...
        if (
            ...
            <= LinhaPerpendicularPoligonoAlgorithm.TRANSPOSE_TRIM_DISTANCE * 2
        ):
            return False

        trimmed_connection_geometry = VectorLayerGeometry.adjust_line_length(
            connection_geometry,
            -LinhaPerpendicularPoligonoAlgorithm.TRANSPOSE_TRIM_DISTANCE,
        )
        if (
    ...
    def _classify_pair_connection_final(
        ...
        for record in connection_records:
            ...
            if LinhaPerpendicularPoligonoAlgorithm._connection_crosses_standardized_layer(
                record.get("geom"),
                standardized_spatial_index,
                standardized_feat_dict,
            ):
        ...
        for grouped_records in grouped_by_father_vertex.values():
            grouped_records.sort(
                key=lambda record: LinhaPerpendicularPoligonoAlgorithm._pair_connection_sort_key(
                    record,
                    record_positions,
                )
            )
        ...
        LinhaPerpendicularPoligonoAlgorithm._apply_crossing_filter_to_valid_connections(
            connection_records,
            record_positions,
        )
